...
func (self *CoordinatorSuite) TestContinuousQueryOperations(c *C) {
	...
	for _, user := range allowedUsers {
		...
		c.Assert(results[0].Id, Equals, uint32(1))
		c.Assert(results[0].Query, Equals, "select * from foo into bar;")

		c.Assert(coordinator.DeleteContinuousQuery(*user, "db1", uint32(1)), IsNil)
    ...
    _, err := coordinator.ListContinuousQueries(*user, "db1")
		...
		c.Assert(coordinator.DeleteContinuousQuery(*user, "db1", uint32(1)), NotNil)
	}
